...
(use-package! eglot
  ...
  (eglot-events-buffer-config '(:size 0 :format full)) ; replaces obsolete eglot-events-buffer-size
  (eglot-extend-to-xref nil)
  ;; Render hover/doc markdown via tree-sitter instead of regex-based
  ;; gfm-view-mode. Requires Eglot >= 1.24 and markdown-ts-mode available
  ;; (built into Emacs 31 core; install md-ts-mode on 29/30).
  (eglot-documentation-renderer 'markdown-ts-view-mode)
  (eglot-ignored-server-capabilities
   '(:documentHighlightProvider        ; avoid overlap with your own highlight/eldoc-box setup
     :documentFormattingProvider       ; apheleia + ruff already own formatting
...
(use-package! eldoc-box
  :after eglot
  :custom
  (eldoc-box-max-pixel-width 800)     ; max width/height of the childframe
  (eldoc-box-max-pixel-height 400)
  (eldoc-box-only-multi-line t)       ; single-line signatures stay in the echo area
  (eldoc-box-fringe-use-same-bg t)    ; no visible seam around the frame
  (eldoc-box-clear-with-C-g t)        ; C-g dismisses like any other popup
  :config
  ;; On-demand only: no :hook enabling eldoc-box-hover-mode or
  ;; eldoc-box-hover-at-point-mode, so the childframe never pops up
  ;; automatically. `eldoc-box-help-at-point' works fine without either
  ;; minor mode enabled, per the package's own docs.
